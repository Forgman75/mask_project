...
@pytest.fixture
def all_same_currency() -> list[dict]:
...
@pytest.fixture
def single_usd_transaction() -> list[dict]:
...
@pytest.fixture
def malformed_transactions() -> list[dict]:
...
@pytest.fixture
def transaction_without_description() -> list[dict]:
    return [
        {"id": 1, "state": "EXECUTED"},
        {"description": "Перевод организации"},
    ]
...
@pytest.fixture
def valid_card_ranges() -> list[tuple]:
...
@pytest.fixture
def expected_card_format() -> str:
    """Ожидаемый формат номера карты"""
    return "XXXX XXXX XXXX XXXX"


@pytest.fixture
def edge_case_ranges() -> list[tuple]:
